...
function generateEmptyBoard() {
    const emptyBoard = [];
    for(let i = 0; i < size; i++)emptyBoard.push([
        0,
        0,
        0,
        0
...
function loseGame() {
    ...
    for(let r = 0; r < size - 1; r++)for(let c = 0; c < size; c++){
...
document.addEventListener("keyup", (events)=>{
    ...
    if (boardChange(board, boardCopy)) addNewTile();
    updateCells();
});
...
function slide(row) {
    ...
    for(let i = 0; i < size - 1; i++)if (newRow[i] === newRow[i + 1] && isFinite(newRow[i])) {
        newRow[i] *= 2;
        newRow[i + 1] = 0;
        score += newRow[i];
    }
...
function move(direction) {
    for(let i = 0; i < size; i++){
        let rowOrColumn;
        if (direction === "left" || direction === "right") rowOrColumn = board[i];
        ...
        if (direction === "right" || direction === "down") rowOrColumn.reverse();
        if (direction === "left" || direction === "right") board[i] = rowOrColumn;
        else if (direction === "up" || direction === "down") for(let j = 0; j < size; j++)board[j][i] = rowOrColumn[j];
    }
}
function moveLeft() {
    move("left");
